...
.controller('indexCtrl', function($scope,$rootScope,sharedUtils,$ionicHistory,$state,$ionicSideMenuDelegate,sharedCartService,fireBaseData,$firebaseArray) {
    ...
    firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
        ...
        $scope.get_lvl= function() {
          var uid = $scope.user_info.uid
          var total_qty="0";
          console.log(uid);


          fireBaseData.refUser().child(uid).child('leveldata').child('level')
      ...
      if(address==null || payment==null){
        ...
        for (var i = 0; i < sharedCartService.cart_items.length; i++) {
          ...
          fireBaseData.refOrder().push({
            ...
            challenge_name: sharedCartService.cart_items[i].challenge_name,
            challenge_price: sharedCartService.cart_items[i].challenge_price,
            challenge_image: sharedCartService.cart_items[i].challenge_image,


            //item dat

            //Order data
            challenge_date: sharedCartService.cart_items[i].challenge_date,
